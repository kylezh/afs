#!/usr/bin/env bash
# ── afs (AgentFS) E2E Test Library ──
# Shared helpers, service startup, and test cases (2-11).
# Sourced by e2e.sh and e2e-nfs.sh.
#
# Caller must set STORAGE_BASE before sourcing this file.
...
# ── Cleanup ──────────────────────────────────────────
# Unmounts FUSE mounts, kills services, removes MOUNT_BASE and DB_PATH.
# Does NOT remove STORAGE_BASE — caller handles that.
...
cleanup() {
    ...
    sleep 0.5
    rm -rf "$MOUNT_BASE" "$DB_PATH" /tmp/afs-controller.toml /tmp/afs-fuse.toml 2>/dev/null
    echo ""
    echo "══════════════════════════"
...
# ── Tests 2-11 ───────────────────────────────────────
# Caller must have registered a filesystem named "testfs" before calling this.
